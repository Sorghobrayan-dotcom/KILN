...
provider, since that would make start-up depend on somebody else's uptime. A
...
def available(st: Settings) -> list[Kind]:
    ...
    never dead on arrival, so a fresh clone can still be driven end to end."""
    ...
    return [
        {
            ...
            "hint": k.hint if k.key in enabled else f"set {k.needs.upper()}, see {k.hint}",
        }
        for k in KINDS
    ]
...
def resolve(st: Settings, key: str) -> Kind:
    ...
    if kind.needs and not getattr(st, kind.needs, ""):
        raise KeyError(f"kind {key!r} needs {kind.needs.upper()}, see {kind.hint}")
    return kind
