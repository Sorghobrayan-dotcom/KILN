...
> I built this for a game studio, because that is where the problem found me.
> The loop it runs suits any team turning out media in quantity: make something,
> judge it, seal it, stop paying twice.
...
**Genblaze** is the orchestration framework: `Pipeline`, `Step`, `SyncProvider`, `ObjectStorageSink`, `StepCache` contract.

**Backblaze B2** carries three distinct surfaces:
...
- `api.py` is the stateless FastAPI surface; reads/writes go to B2 through `B2Blobs`
- `service.py` wires briefs to pipelines to the library
- `forge.py` is the single entry point to Genblaze: builds pipeline, runs, harvests results
- `cache.py` holds `B2StepCache`, the durable cache
- `library.py` does per-project curation: staging, approval, immutable versioned manifests
- `kinds.py` is the modality registry with credential gating
- `memory_backend.py` is an in-memory `StorageBackend` for offline demos
- `pollinations.py` is a Genblaze `SyncProvider` the SDK does not ship

## API

| Method | Path | Auth | |
|---|---|---|---|
| `GET` | `/health` | none | savings counter and kind roster |
| `GET` | `/api/kinds` | none | what this deployment can make, and why not |
| `POST` | `/api/briefs` | token | `{project, description, count, kind}` |
| `GET` | `/api/projects/{p}/staging` | none | everything awaiting a decision |
| `POST` | `/api/projects/{p}/assets/{a}/approve` | token | |
| `POST` | `/api/projects/{p}/assets/{a}/reject` | token | |
| `GET` | `/api/projects/{p}/assets/{a}/provenance` | none | Kiln's record **and** the Genblaze manifest |
| `POST` | `/api/projects/{p}/publish` | token | seals the next version |
| `GET` | `/api/projects/{p}/versions/{n}/manifest` | none | what a build reads |
...
101 tests cover the cache contract, failure paths, curation workflow, and provider
